    ...
    vps:
      hosts:
        server-c:
          ...
          ansible_user: root
          ansible_ssh_pass: "{{ vault_server_c_password }}"
        server-a:
          ansible_host: 203.0.113.10
          ansible_user: deployuser
          ansible_ssh_private_key_file: ~/.ssh/id_ed25519_server_a
...
---
vault_server_c_password: "<your-vault-password>"
...
| `user` | User management | `user: name=deployuser groups=sudo shell=/bin/bash` |
| `authorized_key` | SSH keys | `authorized_key: user=deployuser key="{{ ssh_key }}"` |
